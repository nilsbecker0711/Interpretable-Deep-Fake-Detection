...
class HeatmapEvaluator:
    ...
    def evaluate_heatmap(heatmap, grid_split=3, top_percentile=99.9):
        ...
        if original_size[1] % grid_split != 0 or original_size[0] % grid_split != 0:
            ...
            print(f"Upscaling heatmap from {original_size} to {new_size} for proper grid division.")
        else:
...
class BCOSEvaluator:
    ...
    def evaluate(self, tensor_list, path_list, grid_split):
        ...
        print(f"Processing {len(tensor_list)} grids with grid_split={grid_split}.")
        ...
        for tensor, path in zip(tensor_list, path_list):
            ...
            self.visualize(img_np, heatmap, true_fake_pos, guessed_fake_position, intensity_sums, accuracy, grid_split)
            print(f"Accuracy for {path}: {accuracy:.4f}")
    ...
    def visualize(self, img_np, heatmap, true_fake_pos, guessed_fake_position, intensity_sums, intensity_weighted_accuracy, grid_split):
        ...
        # Draw horizontal grid lines (grid_split - 1 lines)
