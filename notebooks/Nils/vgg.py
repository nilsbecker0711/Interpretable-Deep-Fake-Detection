...
def make_layers(cfg: List[Union[str, int]], batch_norm: bool = False, emb_ch: int = 6, #channels changed from 3 to 6
                no_pool=False) -> nn.Sequential:
...
def _vgg(arch: str, cfg: str, batch_norm: bool, pretrained: bool, progress: bool,
    ...
    model = VGG(make_layers(cfgs[cfg], batch_norm=batch_norm, emb_ch=emb_ch,
                            no_pool=no_pool), **kwargs)
    if pretrained:
