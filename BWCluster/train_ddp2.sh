...
export MASTER_PORT=28700    # The port for communication (can be any available port)

# Launch the training with two GPUs
torchrun --nproc_per_node=2 --nnodes=1 --node_rank=0 --master_addr="localhost" --master_port=28700 ~/Interpretable-Deep-Fake-Detection/training/train.py --detector_path ~/Interpretable-Deep-Fake-Detection/training/config/detector/resnet34_bcos_v2_minimal.yaml --ddp
