...
export MASTER_PORT=29104    # The port for communication (can be any available port)

# Launch the training with two G<PUs
torchrun --nproc_per_node=2 --nnodes=1 --node_rank=0 --master_addr="localhost" --master_port=29104 /BS/robust-architectures/work/bcos-df/training/hp_tuning.py --detector_path /BS/robust-architectures/work/bcos-df/training/config/detector/xception_bcos.yaml --sweep_id 7rtaphxp --ddp
